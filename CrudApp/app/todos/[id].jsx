...
export default function EditScreen() {
    ...
    useEffect(() => {
        ...
        const fetchData = async (id) => {
            try {
                ...
                console.error(`Failed to load todo ${id}`, e);
            }
        }
    ...
    const handleSave = async () => {
        try {
            ...
            const savedTodo = { ...todo, title: todo.title }; // Assume todo state has been updated with new values
            const jsonValue = await AsyncStorage.getItem("TodoApp");
            const storageTodos = jsonValue != null ? JSON.parse(jsonValue) : null;

            if (storageTodos && storageTodos.length > 0) {
                const otherTodos = storageTodos.map(
                    todo => todo.id === savedTodo.id ? savedTodo : todo);
                const allTodos = [...otherTodos, savedTodo];
                await AsyncStorage.setItem("TodoApp", JSON.stringify(otherTodos));
            } else {
            ...
            console.error(`Failed to save todo ${id}`, e);
        }
    }
